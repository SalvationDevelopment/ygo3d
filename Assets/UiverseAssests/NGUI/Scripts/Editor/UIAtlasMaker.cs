...
public class UIAtlasMaker : EditorWindow
{
	...
	static public bool UpdateTexture (UIAtlas atlas, List<SpriteEntry> sprites)
	{
		...
		if (System.IO.File.Exists(newPath))
		{
			...
			{
				debugger.LogError(newPath + " is not editable. Did you forget to do a check out?");
				return false;
			}
#endif
		...
		if (PackTextures(tex, sprites))
		{
			...
			if (newTexture)
			{
				if (tex == null) debugger.LogError("Failed to load the created atlas saved as " + newPath);
		...
		else
		{
			...
			//debugger.LogError("Operation canceled: The selected sprites can't fit into the atlas.\n" +
