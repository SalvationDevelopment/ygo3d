...
static public class FreeType
{
				...
				else
				{
					...
					else
					{
						...
						if (File.Exists(path))
						{
							...
							{
								debugger.LogWarning("Unable to copy " + filename + " to /usr/local/lib:\n" + ex.Message);
							}
						}
					}
				}
	...
	static public string[] GetFaces (Font font)
	{
		...
		{
			debugger.LogError("Failed to initialize FreeType");
			return null;
		}
		...
		if (File.Exists(fileName))
		{
			...
			{
				debugger.LogError("Unable to use the chosen font (FT_New_Face).");
			}
			else
			{
	...
	static public bool CreateFont (Font ttf, int size, int faceIndex, bool kerning, string characters, int padding, out BMFont font, out Texture2D tex)
	{
		...
		if (!File.Exists(fileName))
		{
			debugger.LogError("Unable to use the chosen font.");
		}
		...
		else
		{
			...
			//debugger.Log(ascender + " " + descender + " " + baseline);
