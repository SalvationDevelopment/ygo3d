		...
		if (bytes != null)
		{
			...
			while (reader.canRead)
			{
				...
				if (split[0] == "char")
				{
					...
					{
						debugger.LogError("Your font was exported with more than one texture. Only one texture is supported by NGUI.\n" +
					...
					if (len > 8)
					{
						...
						else debugger.Log("Char: " + split[1] + " (" + id + ") is NULL");
					}
					else
					{
						debugger.LogError("Unexpected number of entries for the 'char' field (" + name + ", " + split.Length + "):\n" + line);
						break;
					}
				}
				else if (split[0] == "kerning")
				{
					...
					{
						debugger.LogError("Unexpected number of entries for the 'kerning' field (" +
							name + ", " + split.Length + "):\n" + line);
						break;
					}
				}
				else if (split[0] == "common")
				{
					...
					if (len > 5)
					{
						...
						if (pages != 1)
						{
							debugger.LogError("Font '" + name + "' must be created with only 1 texture, not " + pages);
							break;
						}
					}
					else
					{
						debugger.LogError("Unexpected number of entries for the 'common' field (" +
							name + ", " + split.Length + "):\n" + line);
						break;
					}
				}
