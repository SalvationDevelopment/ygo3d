...
public class UIRoot : MonoBehaviour
{
	...
	protected virtual void Start ()
	{
		...
		if (oc != null)
		{
			debugger.LogWarning("UIRoot should not be active at the same time as UIOrthoCamera. Disabling UIOrthoCamera.", oc);
	...
	static public void Broadcast (string funcName, object param)
	{
		if (param == null)
		{
			...
			debugger.LogError("SendMessage is bugged when you try to pass 'null' in the parameter field. It behaves as if no parameter was specified.");
		}
		else
		{
			for (int i = 0, imax = list.Count; i < imax; ++i)
