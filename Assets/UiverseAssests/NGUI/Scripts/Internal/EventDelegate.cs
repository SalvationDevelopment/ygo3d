	...
	void Cache ()
	{
		...
		if (mCachedCallback == null || (mCachedCallback.Target as MonoBehaviour) != mTarget || GetMethodName(mCachedCallback) != mMethodName)
		{
			if (mTarget != null && !string.IsNullOrEmpty(mMethodName))
			{
				...
				{
					debugger.LogError("Failed to bind " + type + "." + mMethodName + "\n" +  ex.Message);
					return;
				}
 #else // NETFX_CORE
				...
				if (mMethod == null)
				{
					debugger.LogError("Could not find method '" + mMethodName + "' on " + mTarget.GetType(), mTarget);
					return;
				}
				
				if (mMethod.ReturnType != typeof(void))
				{
					debugger.LogError(mTarget.GetType() + "." + mMethodName + " must have a 'void' return type.", mTarget);
					return;
				}
...
// #endif
		...
		if (list != null)
		{
			for (int i = 0; i < list.Count; )
			{
				...
				if (del != null)
				{
					...
					{
						if (ex.InnerException != null) debugger.LogError(ex.InnerException.Message);
						else debugger.LogError(ex.Message);
					}
#else
					del.Execute();
#endif
	...
	static public EventDelegate Add (List<EventDelegate> list, Callback callback, bool oneShot)
	{
		...
		debugger.LogWarning("Attempting to add a callback to a list that's null");
		return null;
	}
	...
	static public void Add (List<EventDelegate> list, EventDelegate ev, bool oneShot)
	{
		...
		else debugger.LogWarning("Attempting to add a callback to a list that's null");
	}
