...
public class UIWidget : UIRect
{
	...
	public void CheckLayer ()
	{
		...
		{
			//debugger.LogWarning("You can't place widgets on a layer different than the UIPanel that manages them.\n" +
	...
	protected override void OnStart ()
	{
		...
		{
			debugger.LogError("Widgets and panels should not be on the same object! Widget must be a child of the panel.", this);
		}
		else if (!Application.isPlaying && GetComponents<UIWidget>().Length > 1)
		{
			debugger.LogError("You should not place more than one widget on the same object. Weird stuff will happen!", this);
		}
#endif
		CreatePanel();
	}
	...
	public bool UpdateGeometry (int frame)
	{
		...
		if (mChanged)
		{
			if (mIsVisibleByAlpha && finalAlpha > 0.001f && shader != null)
			{
				...
				if (geometry.hasVertices)
				{
					...
					//debugger.Log("Fill " + name + " (" + Time.frameCount + ")");
		...
		else if (mMoved && geometry.hasVertices)
		{
			...
			//debugger.Log("Moving " + name + " (" + Time.frameCount + ")");
