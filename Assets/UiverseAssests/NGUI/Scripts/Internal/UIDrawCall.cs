...
public class UIDrawCall : MonoBehaviour
{
	...
	void CreateMaterial ()
	{
		...
		if (mMaterial != null)
		{
			...
			{
				debugger.LogError(shaderName + " shader doesn't have a clipped shader version for " + mClipCount + " clip regions");
			}
		}
		else
		{
	...
	void UpdateMaterials ()
	{
		...
		{
#if UNITY_EDITOR
			debugger.LogError("Hmm... This point got hit!");
#endif
	...
	public void UpdateGeometry (int widgetCount)
	{
		...
		if (count > 0 && (count == uvs.size && count == cols.size) && (count % 4) == 0)
		{
			...
			else
			{
				...
				debugger.LogError("Too many vertices on one panel: " + verts.size);
			}
		...
		{
			if (mFilter.mesh != null) mFilter.mesh.Clear();
			debugger.LogError("UIWidgets must fill the buffer with 4 vertices per quad. Found " + count);
		}
				...
				if (currentPanel.hasClipping)
				{
					...
					if (currentPanel != panel)
					{
						...
						if (Mathf.Abs(diff.x) > 0.001f || Mathf.Abs(diff.y) > 0.001f)
							debugger.LogWarning("Panel can only be clipped properly if X and Y rotation is left at 0", panel);
