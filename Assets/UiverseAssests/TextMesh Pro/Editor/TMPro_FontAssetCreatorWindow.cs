...
namespace TMPro.EditorUtilities
{
    ...
    public class TMPro_FontAssetCreatorWindow : EditorWindow
    {
        ...
        {
            //debugger.Log("TextMeshPro Editor Window has been disabled.");
        ...
        int[] ParseNumberSequence(string sequence)
        {
            ...
            foreach (string seq in sequences)
            {
                ...
                if (s1.Length == 1)
                    ...
                    {
                        debugger.Log("No characters selected or invalid format.");
                    }
                else
                {
        ...
        void DrawControls()
        {
            ...
            {
                characterSequence = "";
                //debugger.Log("Resetting Sequence!");
            }
            ...
            if (GUILayout.Button("Generate Font Atlas", GUILayout.Width(290)) && characterSequence.Length != 0 && GUI.enabled)
            {
                if (font_TTF != null)
                {
                    ...
                    if (error_Code != 0)
                    {
                        if (error_Code == 99)
                        {
                            //debugger.Log("Font Library was already initialized!");
                            error_Code = 0;
                        }
                        else
                            debugger.Log("Error Code: " + error_Code + "  occurred while Initializing the FreeType Library.");
                    }
                    ...
                    if (error_Code == 0)
                    {
                        ...
                        if (error_Code != 0)
                        {
                            if (error_Code == 99)
                            {
                                //debugger.Log("Font was already loaded!");
                                error_Code = 0;
                            }
                            else
                                debugger.Log("Error Code: " + error_Code + "  occurred while Loading the font.");
                        }
                    }
                    ...
                    if (error_Code == 0)
                    {
                        ...
                        if (error_Code != 0)
                            debugger.Log("Error Code: " + error_Code + "  occurred while Sizing the font.");                      
                    }
                    ...
                    if (error_Code == 0)
                    {
                        ...
                        if (font_CharacterSet_Selection == 6 || font_CharacterSet_Selection == 7)
                        {
                            ...
                            for (int i = 0; i < characterSequence.Length; i++)
                            {
                                ...
                                {
                                    //debugger.Log("Character [" + characterSequence[i] + "] is a duplicate.");
                                }                              
                            }
                        ...
                        ThreadPool.QueueUserWorkItem(SomeTask =>
                        {
                            ...
                            //debugger.Log("Font Rendering is completed.");
                        });
        ...
        void Save_Normal_FontAsset(string filePath)
        {
            ...
            {
                debugger.LogError("You're saving the font asset in a directory outside of this project folder. This is not supported. Please select a directory under \"" + dataPath + "\"");
                return;
            }
            ...
            if (font_asset == null)
            {
                //debugger.Log("Creating TextMeshPro font asset!");
