...
public class TMPro_SDFMaterialEditor : MaterialEditor
{
    ...
    public override void OnEnable()
    {
        ...
        //debugger.Log("New Instance of SDF Material Editor with ID " + this.GetInstanceID());
    ...
    {        
        ...
        {
            //debugger.Log("Material Editor properties have changed. Target is " + target.name); 
        ...
        switch (evt.type)
        {
            ...
            case EventType.DragPerform:
                ...
                if (evt.type == EventType.DragPerform)
                {
                    ...
                    //debugger.Log("Drag-n-Drop Material is " + newMaterial + ". Target Material is " + currentMaterial); // + ".  Canvas Material is " + m_uiRenderer.GetMaterial()  );
                    ...
                    if (!newMaterial || newMaterial == currentMaterial || newMaterial.GetTexture(ShaderUtilities.ID_MainTex).GetInstanceID() != currentMaterial.GetTexture(ShaderUtilities.ID_MainTex).GetInstanceID())
                    {
                        if (newMaterial && newMaterial.GetTexture(ShaderUtilities.ID_MainTex).GetInstanceID() != currentMaterial.GetTexture(ShaderUtilities.ID_MainTex).GetInstanceID())
                            debugger.LogWarning("Drag-n-Drop Material [" + newMaterial.name + "]'s Atlas does not match the Atlas of the currently assigned Font Asset's Atlas.");
                        break;
                    }
    ...
    {
        //debugger.Log("Undo / Redo Event ID (" + Undo.GetCurrentGroup() + ") occured.");
    ...
    {
        /*
        //debugger.Log("Undo Event Registered in SDF Material Editor. # of Properties affected is " + modifications.Length);
        ...
        if (objType == typeof(MeshRenderer) || objType == typeof(Material)) // && UndoEventID != LastUndoEventID)
        {
            ...
            //debugger.Log("OnUndoRedoEvent() received in Material Editor."); // Event ID:" + UndoEventID + ".  Target ID: " + m_modifiedProperties.target.GetInstanceID() + "  Current Material: " + m_modifiedProperties.objectReference + "  New Material: " + (m_modifiedProperties.target as Renderer).sharedMaterial);
            TMPro_EventManager.ON_MATERIAL_PROPERTY_CHANGED(true, target as Material);
        }
