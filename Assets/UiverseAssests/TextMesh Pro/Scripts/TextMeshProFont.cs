...
namespace TMPro
{
    ...
    public class TextMeshProFont : ScriptableObject
    {
        ...
        void OnEnable()
        {
            //debugger.Log("OnEnable has been called on " + this.name);

            if (m_characterDictionary == null)
            {
                //debugger.Log("Loading Dictionary for " + this.name);
                ReadFontDefinition();
            }
            else
            {
                //debugger.Log("Dictionary for " + this.name + " has already been Initialized.");
            }
        }
        ...
        void OnDisable()
        {
            //debugger.Log("TextMeshPro Font Asset [" + this.name + "] has been disabled!");      
        }
        ...
        public void AddGlyphInfo(GlyphInfo[] glyphInfo)
        {
            ...
            {
                //debugger.Log("Glyph Info   x:" + glyphInfo[i].x + "  y:" + glyphInfo[i].y + "  w:" + glyphInfo[i].width + "  h:" + glyphInfo[i].height);
                ...
                //debugger.Log(m_fontInfo.Ascender + "  " + m_fontInfo.Descender);
        ...
        {
            //debugger.Log("Reading Font Definition for " + this.name + ".");
            ...
            //debugger.Log("PRE: BaseLine:" + m_fontInfo.Baseline + "  Ascender:" + m_fontInfo.Ascender + "  Descender:" + m_fontInfo.Descender); // + "  Centerline:" + m_fontInfo.CenterLine);
            ...
            {
                //debugger.Log("Adding Character 32 (Space) to Dictionary for Font (" + m_fontInfo.Name + ").");
            ...
            {
                //debugger.Log("Adding Character 10 (Linefeed) to Dictionary for Font (" + m_fontInfo.Name + ").");
            ...
            {
                //debugger.Log("Adding Character 9 (Tab) to Dictionary for Font (" + m_fontInfo.Name + ").");
            ...
            //debugger.Log(m_fontInfo.Name + " has " + pairs.Count +  " Kerning Pairs.");
            for (int i = 0; i < pairs.Count; i++)
            {
                ...
                if (m_kerningDictionary.ContainsKey(uniqueKey.key) == false)
                    ...
                    debugger.Log("Kerning Key for [" + uniqueKey.ascii_Left + "] and [" + uniqueKey.ascii_Right + "] already exists.");
            }
        ...
        {                      
            ...
            for (int i = 0; i < text.Length; i++)
            {
                ...
                {
                    dict.Add((int)c, c);
                    //debugger.Log("Adding [" + (int)c + "] to dictionary.");
                }
                //else
                //    debugger.Log("Character [" + text[i] + "] is a duplicate.");
            }          
