...
{  
    ...
    public partial class TextMeshProUGUI
    {
        ...
        */
        ...
        {
            //base.Awake();
            //debugger.Log("***** Awake() *****"); // on Object ID:" + GetInstanceID());      
            ...
            if (m_fontAsset == null)
            {
                debugger.LogWarning("Please assign a Font Asset to this " + transform.name + " gameobject.");
                return;
            }
        ...
        {
            //base.OnEnable();
            //debugger.Log("***** OnEnable() *****"); // + GetInstanceID());  // HavePropertiesChanged = " + havePropertiesChanged); // has been called on Object ID:" + gameObject.GetInstanceID());      
            m_isEnabled = true;
        ...
        {
            //base.OnDisable();
            //debugger.Log("***** OnDisable() *****"); //for " + this.name + " with ID: " + this.GetInstanceID() + " has been called.");
            m_isEnabled = false;
        ...
        {
            //base.OnDestroy();
            //debugger.Log("***** OnDestroy() *****");
            
            if (m_maskingMaterial != null)
            {
                //debugger.Log("Trying to release Masking Material [" + m_maskingMaterial.name + "] with ID " + m_maskingMaterial.GetInstanceID());
        ...
        {
            //base.Reset();
            //debugger.Log("***** Reset() *****"); //has been called.");  
            //LoadFontAsset();
            isInputParsingRequired = true;
            havePropertiesChanged = true;
        }
        ...
        {
            //debugger.Log("***** OnTransformParentChanged *****");
        ...
        {
            //debugger.Log("isAwake = " + m_isAwake + "  isEnabled = " + m_isEnabled);
            // Additional Properties could be added to sync up Serialized Properties & Properties.

            //debugger.Log("***** OnValidate() *****"); // ID " + GetInstanceID()); // New Material [" + m_sharedMaterial.name + "] with ID " + m_sharedMaterial.GetInstanceID() + ". Base Material is [" + m_baseMaterial.name + "] with ID " + m_baseMaterial.GetInstanceID() + ". Previous Base Material is [" + (m_lastBaseMaterial == null ? "Null" : m_lastBaseMaterial.name) + "]."); 

            //if (!m_isAwake)
            //    return;
            
            //debugger.Log(m_fontAsset.atlas.GetInstanceID() + "  " + m_uiRenderer.GetMaterial().mainTexture.GetInstanceID());
        ...
        void ON_MATERIAL_PROPERTY_CHANGED(bool isChanged, Material mat)
        {
            ...
            if (m_uiRenderer.GetMaterial() == null)
            {
                if (m_fontAsset != null)
                {
                    ...
                    //debugger.LogWarning("No Material was assigned to " + name + ". " + m_fontAsset.material.name + " was assigned.");
                }
                else
                    debugger.LogWarning("No Font Asset assigned to " + name + ". Please assign a Font Asset.");
            }
           
          
            if (m_uiRenderer.GetMaterial() != m_sharedMaterial && m_fontAsset == null) //    || m_renderer.sharedMaterials.Contains(mat))
            {
                debugger.Log("ON_MATERIAL_PROPERTY_CHANGED Called on Target ID: " + GetInstanceID() + ". Previous Material:" + m_sharedMaterial + "  New Material:" + m_uiRenderer.GetMaterial()); // on Object ID:" + GetInstanceID() + ". m_sharedMaterial: " + m_sharedMaterial.name + "  m_renderer.sharedMaterial: " + m_renderer.sharedMaterial.name);         
            ...
            if (m_stencilID > 0 && m_baseMaterial != null && m_maskingMaterial != null)
            {
                ...
                {
                    //debugger.Log("Copying Material properties from Base Material [" + mat + "] to Masking Material [" + m_maskingMaterial + "].");
                ...
                {                            
                    //debugger.Log("Copying Material properties from Masking Material [" + mat + "] to Base Material [" + m_baseMaterial + "].");
                ...
                {
                    //debugger.Log("Copying Material properties from Masking Material [" + mat + "] to Masking Material [" + m_maskingMaterial + "].");
                    float stencilID = m_maskingMaterial.GetFloat(ShaderUtilities.ID_StencilID);
                    float stencilComp = m_maskingMaterial.GetFloat(ShaderUtilities.ID_StencilComp);
                    m_maskingMaterial.CopyPropertiesFromMaterial(mat);
                    m_maskingMaterial.shaderKeywords = mat.shaderKeywords;
            ...
            //debugger.Log("Assigned Material is " + m_sharedMaterial.name + " with ID " + m_sharedMaterial.GetInstanceID() +
        ...
        void ON_FONT_PROPERTY_CHANGED(bool isChanged, TextMeshProFont font)
        {
            if (font == m_fontAsset)
            {
                //debugger.Log("ON_FONT_PROPERTY_CHANGED event received.");
                havePropertiesChanged = true;
        ...
        {
            debugger.Log("Event Received by " + obj);
            
            if (obj == this)
            {
                //debugger.Log("Undo / Redo Event Received by Object ID:" + GetInstanceID());
                havePropertiesChanged = true;
                isInputParsingRequired = true;
                /* ScheduleUpdate(); */
            }
        ...
        void LoadFontAsset()
        {
            //debugger.Log("***** LoadFontAsset() *****"); //TextMeshPro LoadFontAsset() has been called."); // Current Font Asset is " + (font != null ? font.name: "Null") );
            ShaderUtilities.GetShaderPropertyIDs();

            if (m_fontAsset == null) 
            {
                
                //debugger.LogWarning("No Font Asset has been assigned. Loading Default Arial SDF Font.");

                m_fontAsset = Resources.Load("Fonts & Materials/ARIAL SDF", typeof(TextMeshProFont)) as TextMeshProFont;
                if (m_fontAsset == null)
                {
                    debugger.LogWarning("The ARIAL SDF Font Asset was not found. There is no Font Asset assigned to " + gameObject.name + ".");
                    return;
                }

                if (m_fontAsset.characterDictionary == null)
                {
                    debugger.Log("Dictionary is Null!");
                }
            ...
            else
            {
                if (m_fontAsset.characterDictionary == null)
                {
                    //debugger.Log("Reading Font Definition and Creating Character Dictionary.");
            ...
            if (!m_fontAsset.characterDictionary.TryGetValue(95, out m_cached_Underline_GlyphInfo)) //95
                debugger.LogWarning("Underscore character wasn't found in the current Font Asset. No characters assigned for Underline.");
        ...
        void UpdateEnvMapMatrix()
        {
            ...
            debugger.Log("Updating Env Matrix...");
        ...
        void UpdateMask()
        {
            if (m_rectTransform != null)
            {
                ...
                //debugger.Log("Setting Mask for the first time.");
        ...
        void SetSharedFontMaterial(Material mat) 
        {
            ...
            //debugger.Log("New Material [" + mat.name + "] with ID " + mat.GetInstanceID() + " has been assigned. Base Material is [" + m_baseMaterial.name + "] with ID " + m_baseMaterial.GetInstanceID());
        ...
        {
            debugger.Log("Changing Base Material from [" + (m_lastBaseMaterial == null ? "Null" : m_lastBaseMaterial.name) + "] to [" + mat.name + "].");
        ...
        void SetMeshArrays(int size)
        {
            ...
            //debugger.Log("Size:" + size + "  Vertices:" + m_vertices.Length + "  Triangles:" + m_triangles.Length + " Mesh - Vertices:" + m_mesh.vertices.Length + "  Triangles:" + m_mesh.triangles.Length);
        ...
        void StringToCharArray(string text, ref int[] chars)
        {
            ...
            if (chars.Length <= text.Length)
            {
                ...
                //debugger.Log("Resizing the chars_buffer[" + chars.Length + "] to chars_buffer[" + newSize + "].");
        ...
        {
            //debugger.Log("SetText Array to Char called.");
        ...
        {
            //debugger.Log("Set Array Size called.");
        ...
        {
            //debugger.Log("MarkLayoutForRebuild() called.");
        ...
        {                      
            ...
            if (m_rectTransform != null)
            {              
                //debugger.Log("Computing new margins. Current RectTransform's Width is " + m_rectTransform.rect.width + " and Height is " + m_rectTransform.rect.height); // + "  Preferred Width: " + m_preferredWidth + " Height: " + m_preferredHeight);                            
        ...
        {           
            havePropertiesChanged = true;
            //debugger.Log("Animation Properties have changed.");
        }


        protected override void OnRectTransformDimensionsChange()
        {
            //debugger.Log("**** OnRectTransformDimensionsChange() **** isRebuildingLayout = " + (m_isRebuildingLayout ? "True" : "False") + "."); // Rect: " + m_rectTransform.rect); //  called on Object ID " + GetInstanceID());
            ...
            //debugger.Log("OnRectTransformDimensionsChange() called. New Width: " + m_rectTransform.rect.width + "  Height: " + m_rectTransform.rect.height);
        ...
        {       
            ...
            //debugger.Log("***** OnPreRenderCanvas() ***** Frame: " + Time.frameCount + "  Rect: " + m_rectTransform.rect); // Assigned Material is " + m_uiRenderer.GetMaterial().name); // isInputParsingRequired = " + isInputParsingRequired);                    

            //debugger.Log("Awake = " + m_isAwake + "  Enabled = " + m_isEnabled);
            ...
            {               
                //debugger.Log("RectTransform has changed."); // Current Width: " + m_rectTransform.rect.width + " and  Height: " + m_rectTransform.rect.height);
                ...
                {                    
                    //debugger.Log("RectTransform Dimensions or Margins have changed.");
                    ComputeMarginSize();
            ...
            {
                //debugger.Log("Properties have changed!"); // Assigned Material is:" + m_sharedMaterial); // New Text is: " + m_text + ".");                
                ...
                {
                    //debugger.Log("Font Asset has changed. Loading new font asset."); 
                ...
                //    {
                        //LayoutRebuilder.MarkLayoutForRebuild(m_rectTransform);
                        //debugger.Log("Calling MarkLayoutForRebuild() at Frame: " + Time.frameCount);
                        //return;
                //    }
                //}

                //if (m_isLayoutDirty) { debugger.Log("Regenerating Text since layout was dirty."); }
        ...
        void GenerateTextMesh()
        {
            ...
            {
                //debugger.Log("Early Out! No Text has been set.");
            ...
            //debugger.Log (corners [0] + "  " + corners [2]);
            ...
            for (int i = 0; m_char_buffer[i] != 0; i++)
            {
                ...
                //debugger.Log("i:" + i + "  Character [" + (char)charCode + "] with ASCII of " + charCode);
                ...
                //    {
                //        debugger.Log("Loading secondary font asset.");
                ...
                //    }
                //}               
                //debugger.Log("Char [" + (char)charCode + "] is using FontIndex: " + m_fontIndex);
                ...
                else
                {
                    ...
                    if (m_cached_GlyphInfo == null)
                    {
                        ...
                        if (m_cached_GlyphInfo == null)
                        {
                            ...
                            if (m_cached_GlyphInfo != null)
                            {
                                debugger.LogWarning("Character with ASCII value of " + charCode + " was not found in the Font Asset Glyph Table.");
                        ...
                        if (enableWordWrapping && m_characterCount != m_firstVisibleCharacterOfLine)
                        {
                            ...
                            if (wrappingIndex == m_SavedWordWrapState.previous_WordBreak || isFirstWord)
                            {
                                ...
                                if (m_isCharacterWrappingEnabled == false)
                                {
                                    ...
                                    //debugger.Log("Enabling Character Wrapping.");                                                                  
                                }
                                else
                                    isLastBreakingChar = true;

                                
                                //debugger.Log("Warpping Index " + wrappingIndex + ". Recursive Count: " + m_recursiveCount);

                                m_recursiveCount += 1;
                                if (m_recursiveCount > 20)
                                {
                                    debugger.Log("Recursive count exceeded!");
                                    continue;
                                }

                                //debugger.Log("Line #" + m_lineNumber + " Character [" + (char)charCode + "] cannot be wrapped.  WrappingIndex: " + wrappingIndex + "  Saved Index: " + m_SavedWordWrapState.previous_WordBreak + ". Character Count is " + m_characterCount);                               
                            }
                            ...
                            //debugger.Log("Last Visible Character of line # " + m_lineNumber + " is [" + m_textInfo.characterInfo[m_lastVisibleCharacterOfLine].character + " Character Count: " + m_characterCount + " Last visible: " + m_lastVisibleCharacterOfLine);
                            ...
                            //debugger.Log("LineInfo for line # " + (m_lineNumber) + " First character [" + m_textInfo.characterInfo[m_textInfo.lineInfo[m_lineNumber].firstCharacterIndex].character + "] at Index: " + m_textInfo.lineInfo[m_lineNumber].firstCharacterIndex +
                ...
                {
                    //debugger.Log((m_maxAscender - m_maxDescender) + "  " + marginHeight);
                    //debugger.Log("Character [" + (char)charCode + "] at Index: " + m_characterCount + " has exceeded the Height of the text container. Max Ascender: " + m_maxAscender + "  Max Descender: " + m_maxDescender + "  Margin Height: " + marginHeight + " Bottom Left: " + bottom_left.y);                                              
                    ...
                    switch (m_overflowMode)
                    {
                        ...
                        case TextOverflowModes.Truncate:
                         if (m_isMaskingEnabled)
                            ...
                            //    {
                            //        debugger.Log("Char [" + (char)charCode + "] on line " + m_lineNumber + " exceeds the vertical bounds. Last safe character was " + (int)m_textInfo.characterInfo[ellipsisIndex + 1].character);
                            //        i = RestoreWordWrappingState(ref m_SavedWordWrapState);
                            //        m_lineNumber -= 1;
                            //        m_isTextTruncated = true;
                            //        m_isCharacterWrappingEnabled = true;
                            ...
                            //    else
                            //    {
                            //        //debugger.Log("Char [" + (char)charCode + "] on line " + m_lineNumber + " set to invisible.");
                            //        m_textInfo.characterInfo[m_characterCount].isVisible = false;
                            //    }
                            ////    m_char_buffer[m_textInfo.characterInfo[ellipsisIndex].index + 1] = (char)0;
                            ////    m_isTextTruncated = true;
                        ...
                        case TextOverflowModes.Page:
                            ...
                            //debugger.Log("Character is [" + (char)charCode + "] with ASCII (" + charCode + ") on Page " + m_pageNumber + ". Ascender: " + m_textInfo.pageInfo[m_pageNumber].ascender + "  BaseLine: " + m_textInfo.pageInfo[m_pageNumber].baseLine + "  Descender: " + m_textInfo.pageInfo[m_pageNumber].descender);                          
                ...
                {
                    //debugger.Log("Line # " + m_lineNumber + "  Current Character is [" + (char)charCode + "] with ASC value of " + charCode);

                    // Handle Line Spacing Changes
                    if (m_lineNumber > 0 && m_maxFontScale != 0 && m_lineHeight == 0 && m_maxFontScale != previousFontScale && !m_isNewPage)
                    {
                        //debugger.Log("Adjusting Line Spacing");
                        float gap = m_fontAssetArray[m_fontIndex].fontInfo.LineHeight - (m_fontAssetArray[m_fontIndex].fontInfo.Ascender - m_fontAssetArray[m_fontIndex].fontInfo.Descender);
                        float offsetDelta = (m_fontAssetArray[m_fontIndex].fontInfo.Ascender + m_lineSpacing + m_paragraphSpacing + gap + m_lineSpacingDelta) * m_maxFontScale - (m_fontAssetArray[m_fontIndex].fontInfo.Descender - gap) * previousFontScale;
                        m_lineOffset += offsetDelta - lineOffsetDelta;
                    ...
                    float lineAscender2 = (m_fontAsset.fontInfo.Ascender + m_alignmentPadding.y) * m_fontScale - m_lineOffset + m_baselineOffset;
                    lineAscender = lineAscender > lineAscender2 ? lineAscender : lineAscender2;
                    //debugger.Log("Line # " + m_lineNumber + " -- Current Page is " + m_pageNumber + ". Previous Page is " + previousPageNumber);
                    ...
                    //debugger.Log("Line # " + m_lineNumber + " XAdance is " +  (m_preferredWidth + m_xAdvance + (m_alignmentPadding.z * m_fontScale)) + "  Max XAdvance: " + m_maxXAdvance);
                ...
                if (charCode != 13 && charCode != 10 && m_pageNumber < 16)
                {
                    ...
                    //debugger.Log("Char [" + (char)charCode + "] with ASCII (" + charCode + ") on Page # " + m_pageNumber + " with Ascender: " + m_textInfo.pageInfo[m_pageNumber].ascender + ". Descender: " + m_textInfo.pageInfo[m_pageNumber].descender);
                ...
                {            
                    if (charCode == 9 || charCode == 32) // || char.IsPunctuation((char)charCode))
                    {
                        ...
                        m_isCharacterWrappingEnabled = false;
                        isFirstWord = false;
                        //debugger.Log("Storing Word Wrapping Info at CharacterCount " + m_characterCount);             
                    }
                    ...
                    {
                        //debugger.Log("Storing Character [" + (char)charCode + "] at Index: " + i);
                        SaveWordWrappingState(ref m_SavedWordWrapState, i, m_characterCount);
                    }
            ...
            //debugger.Log("Iteration Count: " + loopCountA + ". Final Point Size: " + m_fontSize); // + "  B: " + loopCountB + "  C: " + loopCountC + "  D: " + loopCountD);
            ...
            switch (m_textAlignment)
            {
                ...
                case TextAlignmentOptions.TopRight:
                    ...
                    else
                    {
                        ...
                        //debugger.Log("Page # " + m_pageToDisplay + " Ascender is " + m_textInfo.pageInfo[m_pageToDisplay].ascender);
                    }
                    break;
            ...
            for (int i = 0; i < m_characterCount; i++)
            {
                    ...
                    case TextAlignmentOptions.MidlineJustified:
                        ...
                        {   // All lines are justified accept the last one.
                            ...
                            else
                            {
                                ...
                                {
                                    //debugger.Log("LineInfo Character Count: " + lineInfo.characterCount);
                        ...
                        //debugger.Log("Char [" + (char)charCode + "] Code:" + charCode + "  Offset:" + justificationOffset + "  # Spaces:" + m_lineExtents[currentLine].NumberOfSpaces + "  # Characters:" + m_lineExtents[currentLine].NumberOfChars);                       
                        break;
                }
                ...
                if (characterInfoArray[i].isVisible)
                {
                    ...
                    switch (type)
                    {
                        ...
                        case TMP_CharacterType.Character:
                            ...
                            {                               
                                ...
                                case TextureMappingOptions.MatchAspect:
                                    ...
                                    switch (m_verticalMapping)
                                    {
                                        ...
                                        case TextureMappingOptions.MatchAspect:
                                            debugger.Log("ERROR: Cannot Match both Vertical & Horizontal.");
                                            break;
                                    }
                            ...
                            //debugger.Log("Camera Assigned = " + (m_canvas.worldCamera != null) + ".  Character Scale: " + characterInfoArray[i].scale + ". LossyScale: " + m_rectTransform.lossyScale.z + ".  xScale: " + xScale + ". Canvas Scale: " + m_canvas.scaleFactor);
                ...
                else if ((char.IsPunctuation(currentCharacter) || char.IsWhiteSpace(currentCharacter) || i == m_characterCount - 1) && isStartOfWord || i == 0)
                {
                    ...
                    //debugger.Log("Word #" + wordCount + " is [" + wordInfo.word + "] Start Index: " + wordInfo.firstCharacterIndex + "  End Index: " + wordInfo.lastCharacterIndex);
                }
                #endregion
                ...
                if (isUnderline)
                {
                    if (beginUnderline == false && currentCharacter != 32 && currentCharacter != 10 && currentCharacter != 13)
                    {
                        ...
                        //debugger.Log("Underline Start Char [" + m_textInfo.characterInfo[i].character + "].");
                    }
            ...
            {
                //debugger.Log("Uploading Mesh normally.");
                // Upload Mesh Data 
            ...
            {
                //debugger.Log("Auto-fitting Text. Default Width:" + m_textContainer.isDefaultWidth + "  Default Height:" + m_textContainer.isDefaultHeight);
                if (marginWidth == 0)
                ...
                debugger.Log("Auto-fitting Text. Default Width:" + m_preferredWidth + "  Default Height:" + m_preferredHeight);
                GenerateTextMesh();
                return;
            }
            */
            ...
            //{
            //    debugger.Log("Line: " + (i + 1) + "  # Char: " + m_textInfo.lineInfo[i].characterCount
            ...
            //debugger.Log("Done rendering text. Character Count is " + m_textInfo.characterCount);
            //debugger.Log("Done rendering text. Preferred Width:" + m_preferredWidth + "  Preferred Height:" + m_preferredHeight);

            //debugger.Log(m_minWidth);
            //Profiler.EndSample();
            //m_StopWatch.Stop();           
            //debugger.Log("Done Rendering Text.");

//#if UNITY_EDITOR
//            UnityEditorInternal.InternalEditorUtility.RepaintAllViews();
//#endif

            //debugger.Log("TimeElapsed is:" + (m_StopWatch.ElapsedTicks / 10000f).ToString("f4"));
            //m_StopWatch.Reset();     
        }
        ...
        void FillSpriteVertexBuffers(int i, int spriteIndex_X4)
        {
            ...
            //debugger.Log(m_visibleSpriteCount);          
        ...
        void DrawUnderlineMesh(Vector3 start, Vector3 end, ref int index, Color32 underlineColor)
        {
            ...
            //debugger.Log("Char H:" + cached_Underline_GlyphInfo.height);
        ...
        void ResizeMeshBuffers(int size)
        {
            ...
            //debugger.Log("Resizing Mesh Buffers from " + previousSize + " to " + size + ".");
        ...
        bool ValidateHtmlTag(int[] chars, int startIndex, out int endIndex)
        {
            ...
            //debugger.Log("Tag is [" + m_htmlTag.ArrayToString() + "].  Tag Code:" + tagCode + "  Color Code: " + colorCode);
                ...
                switch (tagCode)
                {
                    ...
                    case 1019: // <page>
                        ...
                        if (m_overflowMode == TextOverflowModes.Page)
                        {
                            ...
                            m_pageNumber += 1;
                            m_isNewPage = true;
                            //debugger.Log("m_lineOffet is " + m_lineOffset);
                        }
                        return true;
                    case 1020: // </sub>
                    ...
                    case 1118: // <font=xx>
                        ...
                        if(m_fontAssetArray[m_fontIndex] == null)
                        {
                            ...
                            //debugger.Log("Loading Font Asset at Index " + m_fontIndex);
                        ...
                        {
                            //debugger.Log("Font Asset at Index " + m_fontIndex + " has already been loaded.");
                        }
                    ...
                    case 2287: // <sprite=x>
                        ...
                        //debugger.Log("Sprite Index is " + ((int)ConvertToFloat(m_htmlTag, numSequenceStart, tagCharCount - 1, numSequenceDecimalPos)));
                        m_isSprite = true;
                        return true;
                    case 2844: // </mspace>
                        m_monoSpacing = 0;
