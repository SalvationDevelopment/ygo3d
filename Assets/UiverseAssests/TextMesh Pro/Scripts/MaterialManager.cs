...
namespace TMPro
{
    ...
    public static class MaterialManager
    {
        ...
        public static Material GetStencilMaterial(Material baseMaterial, int stencilID)
        {
            ...
            {
                debugger.LogWarning("Selected Shader does not support Stencil Masking. Please select the Distance Field or Mobile Distance Field Shader.");
                return baseMaterial;
            }
            ...
            if (index == -1)
            {
                ...
                //debugger.Log("Masking material for " + baseMaterial.name + " DOES NOT exists. Creating new " + maskingMaterial.name + " with ID " + maskingMaterial.GetInstanceID() + " which is used " + temp.count + " time(s).");           
            ...
            else
            {
                ...
                //debugger.Log("Masking material for " + baseMaterial.name + " already exists. Passing reference to " + maskingMaterial.name + " with ID " + maskingMaterial.GetInstanceID() + " which is used " + m_materialList[index].count + " time(s).");           
            }
        ...
        public static void ReleaseBaseMaterial(Material baseMaterial)
        {
            ...
            if (index == -1)
            {
                debugger.Log("No Masking Material exists for " + baseMaterial.name);
            }
            else
            {
                ...
                {
                    m_materialList[index].count -= 1;
                    debugger.Log("Removed (1) reference to " + m_materialList[index].stencilMaterial.name + ". There are " + m_materialList[index].count + " references left.");
                }
                else
                {
                    debugger.Log("Removed last reference to " + m_materialList[index].stencilMaterial.name + " with ID " + m_materialList[index].stencilMaterial.GetInstanceID());
        ...
        public static void ReleaseStencilMaterial(Material stencilMaterial)
        {
            ...
            if (index == -1)
            {
                //debugger.Log("No Masking Material exists for " + maskingMaterial.name);
            }
            else
            {
                if (m_materialList[index].count > 1)
                {
                    m_materialList[index].count -= 1;
                    //debugger.Log("Removed (1) reference to " + m_materialList[index].maskingMaterial.name + ". There are " + m_materialList[index].count + " references left.");
                }
                else
                {
                    //debugger.Log("Removed last reference to " + m_materialList[index].maskingMaterial.name + " with ID " + m_materialList[index].maskingMaterial.GetInstanceID());
                    Object.DestroyImmediate(m_materialList[index].stencilMaterial);
                    m_materialList.RemoveAt(index);
                }
            }
        ...
        public static void ClearMaterials()
        {
            ...
            {
                debugger.Log("Material List has already been cleared.");
                return;
            }
        ...
        public static void ListMaterials()
        {
            ...
            if (m_materialList.Count() == 0)
            {
                debugger.Log("Material List is empty.");
                return;
            }

            //debugger.Log("List contains " + m_materialList.Count() + " items.");
